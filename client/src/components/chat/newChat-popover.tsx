    ...
    <Popover open={isOpen} onOpenChange={handleOpenChange}>
      <PopoverTrigger>
        <Button
          ...
          className="h-8 w-8"
        >
          <PenBoxIcon className="h-8! w-5! stroke-1!" />
        </Button>
      </PopoverTrigger>
      <PopoverContent
        align="start"
      ...
      >
        <div className="border-b p-3 flex flex-col gap-2">
          <div className="flex items-center gap-2">
            {isGroupMode && (
              <Button variant="ghost" size="icon" onClick={handelBack}>
                <ArrowLeft size={16} />
              </Button>
            )}
...
const ChatUserItem = memo(
  ({
    ...
    return (
      <button
        className="
      relative w-full flex items-center gap-2 p-2 rounded-sm hover:bg-accent transition-colors text-left disabled:opacity-50
      "
